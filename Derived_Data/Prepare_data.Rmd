...
> Cape Small: Cape Small is the eastern most region of the bay. It includes the
...
data on analysis of toxic compounds in Porland Harbor sediments, available at
...
parts per billion, the latter to parts per trillion.
...
well-defined sets of compounds need to be summed. The result is a number of
"totals" that were used in historical analyses, but that are not of value here.
 

## PAH Data
Three historic PAH "Totals" were included in the data.  FOr analysis or
reanalysis of raw data, we do not want to include them, so we removbe them here.
```{r clean_pah_data}
...
Organic Carbon is in the dataset (where it is available at all) in two different
sets of units.  We retain only the version that is in percent, for the
simplicity of making sure that all the units in this dataset are the same.
...
### Simplify Data
Many of the remaining data columns have little meaning n the context of the 
